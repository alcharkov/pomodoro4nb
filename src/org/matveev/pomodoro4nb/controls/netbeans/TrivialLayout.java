...
final class TrivialLayout implements LayoutManager {
    ...
    private void layoutComp(Container parent) {
        ...
        if (c.length > 1) {
            ...
            if (parent.getWidth() - d1.width < d2.width) {
                ...
                c[1].setBounds(d1.width + 1, 0, parent.getWidth()
                        - d1.width, Math.min(parent.getHeight(),
                        d2.height));
            }
        }
    }
    ...
    private void layoutTapPanel(TapPanel tp) {
        ...
        if (c.length > 1) {
            ...
            if (tp.isExpanded()) {
                ...
                int height = Math.min(tp.getHeight()
                        - tp.getMinimumHeight(), d2.height);
                if (tp.getWidth() - d1.width < d2.width) {
                    c[0].setBounds(0, 0, 0, 0);
                    c[1].setBounds(0, top, tp.getWidth(), height);
                } else {
                    c[0].setBounds(0, top + labely, d1.width,
                            d1.height);
                    c[1].setBounds(d1.width + 1, top, tp.getWidth()
                            - d1.width, height);
                }
            } else {
                c[0].setBounds(0, 0, 0, 0);
    ...
    public Dimension minimumLayoutSize(Container parent) {
        ...
        if (c.length > 1) {
            ...
            result.height = tp.isExpanded() ? Math.max(d1.height,
                    d2.height)
                    + tp.getMinimumHeight() : tp.getMinimumHeight();
        }
        return result;
    }
